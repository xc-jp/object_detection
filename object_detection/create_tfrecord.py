...
def split(df, group) -> list:
    data = namedtuple("data", ["filename", "object"])
    gb = df.groupby(group)
    return [data(filename, gb.get_group(x)) for filename, x in zip(gb.groups.keys(), gb.groups, strict=False)]
...
def main(_):
    ...
    with open(path_to_labeltxt) as f:
        labels = [line.strip() for line in f.readlines()]

    path_to_labelpbtxt = os.path.join(os.path.dirname(FLAGS.output_path), "labelmap.pbtxt")
